...
class AgentFrameworkProductManagementAgent:
    ...
    def _get_agent_response(
        ...
        try:
            ...
            # If the message is not valid JSON, treat it as a completed plain text response
            logger.debug("Response is not JSON, treating as plain text: %s", message[:100] if message else "empty")
            return {
